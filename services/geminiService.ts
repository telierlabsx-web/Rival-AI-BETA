...
💻 **CODING:**
- Bisa semua bahasa & framework: React, Vue, Node.js, Python, dll
- SELALU kasih code LENGKAP & WORKING
...
export const geminiService = {
  async chat(
    ...
    try {
      ...
      // 🔥 SIMPLE MODE-SPECIFIC INSTRUCTIONS (NO LONG CODING PROMPT!)
      if (isCanvasMode && codeIntent === 'CODE_CREATION') {
        systemPrompt += `\n\n🎨 CANVAS MODE - CODE CREATION:
- Penjelasan SINGKAT (1-2 kalimat)
- Code LENGKAP di tsx/jsx (biar bisa di-preview)
- Production-ready, NO TODO/placeholder
- Modern UI dengan Tailwind`;
        console.log('🎨 [CANVAS] Simple coding instruction');
      } else if (isThinkingMode) {
        systemPrompt += `\n\n💭 THINKING MODE:
- Chat santai, natural
- JANGAN nawarin bikin code kecuali user MINTA
- Kalau user minta code, langsung kasih
- Keep it simple`;
        console.log('💭 [THINKING] Casual chat mode');
      }
