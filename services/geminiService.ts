...
- **Real-time Maps**: Satellite view & standard maps (masih dalam pengembangan)
...
export const geminiService = {
  ...
  async chat(
    ...
    useAutoMode: boolean = false // PARAMETER BARU
  ) {
    try {
      if (keyPool.length === 0) initKeyPool();
      ...
      console.log(`💬 [CHAT] Using gemini-2.5-flash with ${useAutoMode ? selectedMode.toUpperCase() : 'DEFAULT'} mode...`);
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          ...modeConfig // Apply temperature & settings
        }
      });
      ...
      return { 
        ...
        sources,
        mapData,
        aiMode: useAutoMode ? selectedMode : undefined // Return mode yang dipilih
      };
