...
User: "Lokasi Candi Borobudur"
Response: "Candi Borobudur adalah candi Buddha terbesar di dunia yang terletak di Magelang, Jawa Tengah. Dibangun pada abad ke-9, candi ini merupakan situs warisan dunia UNESCO.
...
User: "Dimana Menara Eiffel?"
Response: "Menara Eiffel adalah menara besi ikonik di Paris, Prancis. Dibangun tahun 1889 untuk Pameran Dunia, tingginya 330 meter dan jadi simbol Prancis yang paling terkenal.

[LOCATION_DATA]
{
  "title": "Menara Eiffel",
  "location": "Champ de Mars, Paris, Prancis",
  "latitude": 48.8584,
  "longitude": 2.2945
}
[/LOCATION_DATA]"
...
**PENTING:**
- Selalu kasih koordinat AKURAT (latitude/longitude dalam desimal)
- Format JSON harus STRICT (double quotes, valid JSON)
- Kasih penjelasan dulu, baru koordinat
- Jangan kasih koordinat kalo ga yakin 100%
...
📋 **SELF-INTRODUCTION (Kalo ditanya siapa kamu/tentang dirimu):**
"Gue Rival - AI assistant yang dikembangkan sama Muhamad Rivaldy untuk vCrop. Gue dibangun dari September 2024 sampe Januari 2025, sekarang lagi fase beta 2025-2026. Gue powered by Gemini model terbaik dari Google, bisa coding full-stack, search real-time, liat maps OpenStreetMap dengan dark mode, dan punya mode offline pake model Qwen. Intinya, gue di sini buat bantu lo semaksimal mungkin dengan cara yang asik dan natural!"

(Tapi jangan mention ini kalo ga ditanya - stay natural aja!)`;

// 🔥 CONFIG UNTUK FAST & EXPERT MODE
...
// 🗺️ EXTRACT MAP DATA dari AI response
const extractMapData = (text: string): { cleanText: string; mapData?: MapData } => {
  ...
  try {
    ...
    if (
      ...
      // Remove location data dari text
...
export const geminiService = {
  // FUNGSI UTAMA dengan AUTO MODE + CANVAS MODE
  async chat(
    messages: Message[], 
    userPersona: string, 
    ...
    try {
      ...
      // 🔥 AUTO MODE: Deteksi kompleksitas pesan terakhir
      ...
      // 🎨 CANVAS MODE: Deteksi code intent
      ...
      // 🚀 ENHANCED PROMPT: Inject coding rules kalau butuh
      ...
      // 🔥 Apply config based on auto mode
      const modeConfig = useAutoMode ? getModeConfig(selectedMode) : {};

      // Call AI with ALL tools enabled
      ...
      // 📝 Extract code if present (IMPROVED REGEX)
      let codeSnippet = "";
      ...
      if (codeMatch) {
        ...
        // Validate code quality (basic check)
      ...
      // 🗺️ Prioritize AI-generated map data over grounding metadata
