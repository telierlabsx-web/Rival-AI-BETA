...
export const geminiService = {
  async chat(
    ...
    try {
      ...
      let systemInstruction = `ANDA ADALAH RIVAL (BETA DEPLOYMENT). ROLE: MASTER ENGINEER & VISUAL ARCHITECT.
...
- ${userPersona || "Fokus pada efisiensi sistem dan kualitas output."}`;
      ...
      // IMAGE GENERATION - PAKAI MODEL YANG BENER
      if (isImageRequest && !images?.length) {
        const response = await ai.models.generateContent({
          model: 'gemini-2.5-flash-image', // ✅ MODEL YANG BENER
      ...
      // TEXT/CODE/SEARCH - PAKAI MODEL YANG BENER
      let modelName = 'gemini-2.5-flash'; // ✅ DEFAULT MODEL YANG BENER
      
      // Untuk tugas kompleks, pake yang lebih powerful
      if (isEbookRequest || isCodeRequest) {
        modelName = 'gemini-3-pro-preview'; // ✅ MODEL PRO YANG BENER
      }
      ...
      throw new Error(`Rival mengalami gangguan teknis: ${error.message || 'Unknown error'}`);
    }
  }
};
