...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    <div 
      className={`fixed bottom-0 right-0 p-3 md:p-4 pointer-events-none z-10 transition-all duration-300 ease-in-out ${
    ...
    >
      <div className="max-w-3xl mx-auto w-full pointer-events-auto space-y-2">
        ...
        <div className={`relative ${colors.bg} rounded-[30px] border ${colors.border} shadow-xl`}>
          ...
          {/* ✅ FIX: Padding konsisten + min-height */}
          <div className="p-4 md:p-5">
            {/* ✅ FIX: Input dengan min-height */}
            <input
              ref={inputRef}
              type="text"
              value={input}
              ...
              className={`w-full bg-transparent focus:outline-none text-[15px] md:text-base mb-4 ${colors.text} placeholder:${colors.textMuted} min-h-[24px]`}
              style={{ minHeight: '24px' }}
            />

            {/* ✅ FIX: Flexbox proper dengan min-height */}
            <div className="flex items-center gap-2 min-h-[40px]">
              <button 
                onClick={() => setIsAttachmentOpen(!isAttachmentOpen)} 
                className={`p-2 rounded-full ${isAttachmentOpen ? colors.activeBg : colors.hover} transition-all flex-shrink-0`}
              >
              ...
              <button 
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-full ${colors.hover} transition-all flex-shrink-0 min-h-[36px]`}
              >
                {getModeIcon()}
              ...
              <button 
                ...
                className={`p-3 rounded-full ${colors.sendBg} ${colors.sendText} shadow-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0 min-w-[44px] min-h-[44px] flex items-center justify-center`}
              >
                {isLoading ? (
