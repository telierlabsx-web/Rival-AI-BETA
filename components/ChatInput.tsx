...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  useEffect(() => {
    if (textareaRef.current) {
      ...
      const newHeight = Math.min(textareaRef.current.scrollHeight, 24 * 4); // Max 4 lines (24px per line)
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // Enter alone = send message (only if there's text)
    ...
    >
      <div className="max-w-3xl mx-auto w-full pointer-events-auto space-y-3">
        ...
        <div className={`relative ${colors.bg} rounded-[30px] border ${colors.border} shadow-xl`}>
          ...
          {/* ✅ FIX: Proper padding for mobile & desktop */}
          <div className="p-4 md:p-5">
            {/* ✅ FIX: Textarea dengan auto-grow (max 4 lines) + proper line-height */}
            <textarea
              ref={textareaRef}
              value={input}
              ...
              rows={1}
              className={`w-full bg-transparent focus:outline-none text-[15px] md:text-base mb-3 ${colors.text} placeholder:${colors.textMuted} resize-none overflow-y-auto`}
              style={{ 
                minHeight: '24px',
            ...
            {/* ✅ FIX: Bottom controls with proper spacing */}
            <div className="flex items-center gap-2">
              <button 
                onClick={() => setIsAttachmentOpen(!isAttachmentOpen)} 
                className={`p-2.5 rounded-full ${isAttachmentOpen ? colors.activeBg : colors.hover} transition-all flex-shrink-0`}
              >
              ...
              <button 
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-full ${colors.hover} transition-all flex-shrink-0`}
              >
                {getModeIcon()}
                <span className={`text-[13px] font-medium ${colors.text} hidden sm:inline`}>Grok 4.1 Thinking</span>
              ...
              <button 
                ...
                className={`p-3 rounded-full ${colors.sendBg} ${colors.sendText} shadow-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0 w-11 h-11 flex items-center justify-center`}
              >
                {isLoading ? (
        ...
        {/* ✅ Helper text untuk keyboard shortcuts */}
        <div className={`text-center text-xs ${colors.textMuted} px-2`}>
          <span className="hidden md:inline">Tekan Enter untuk kirim • Shift+Enter untuk baris baru</span>
          <span className="md:hidden">Enter untuk kirim pesan</span>
        </div>
      </div>
    </div>
  );
