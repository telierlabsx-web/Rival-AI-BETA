  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 p-4 md:p-8 bg-gradient-to-t from-inherit via-inherit/98 to-transparent pointer-events-none">
      <div className="max-w-3xl mx-auto w-full pointer-events-auto space-y-3">
        ...
        {/* MAIN INPUT BOX - KAYAK GROK */}
        <div className={`relative rounded-[32px] border-2 ${colors.bg} ${colors.border} shadow-2xl transition-all overflow-hidden`}>
          
          {/* DROPDOWN MODE SELECTOR - MUNCUL DARI ATAS */}
          {isModeOpen && (
            <div className={`p-3 border-b-2 ${colors.menuBorder} animate-in fade-in slide-in-from-top-2`}>
              {/* SUPER RIVAL UPGRADE */}
              <div className={`flex items-center justify-between px-4 py-3 rounded-[20px] ${colors.hover} mb-2`}>
                ...
                <button className={`px-3 py-1.5 rounded-full ${colors.activeBg} ${colors.activeText} text-[10px] font-black uppercase hover:scale-105 transition-transform`}>
                  Upgrade
                </button>
              </div>
          ...
          <div className="flex items-end gap-2 p-2.5">
            ...
            {/* MODE SELECTOR BUTTON - KIRI BAWAH */}
            <button
              onClick={() => setIsModeOpen(!isModeOpen)}
              className={`flex-shrink-0 flex items-center gap-2 px-3.5 py-2.5 rounded-full ${colors.hover} transition-all`}
            >
              {getModeIcon(activeMode)}
              <span className={`text-xs font-bold ${colors.text}`}>{getModeLabel(activeMode)}</span>
            ...
            <textarea
              ...
              className={`flex-1 bg-transparent py-4 px-2 focus:outline-none text-base font-medium max-h-60 min-h-[48px] overflow-y-auto no-scrollbar resize-none ${colors.text}`}
              rows={1}
            />
            ...
            <button 
              ...
              className={`flex-shrink-0 p-4 rounded-[20px] transition-all ${(input.trim() || (selectedImages && selectedImages.length > 0)) ? `${colors.activeBg} ${colors.activeText}` : 'opacity-10'}`}
            >
              {isLoading ? (
