...
interface ChatInputProps {
  ...
  isAutoMode: boolean; // 🔥 TAMBAH
  setIsAutoMode: (value: boolean) => void; // 🔥 TAMBAH
  isDark: boolean;
  onSend: () => void;
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  activeMode, 
  setActiveMode, 
  isAutoMode, // 🔥 TERIMA PROP
  setIsAutoMode, // 🔥 TERIMA PROP
  isDark, 
  onSend
}) => {
  ...
  // 🔥 AUTO MODE: Sync activeMode dengan isAutoMode
  useEffect(() => {
    if (activeMode === 'auto') {
      setIsAutoMode(true);
    } else {
  ...
  }, [activeMode, setIsAutoMode]);

  return (
    <div className="fixed bottom-0 left-0 right-0 z-[100] pb-[env(safe-area-inset-bottom)]">
      <div className="max-w-3xl mx-auto px-4 py-3 mb-4 relative">
          ...
          <div className="flex flex-col p-2">
            ...
            <div className="flex items-center justify-between px-2 pb-1 mt-1">
              <div className="flex items-center gap-1.5">
                ...
                >
                  <span className={isAutoMode ? 'text-white' : 'text-zinc-400'}>{currentMode.icon}</span>
                  <span className={`text-[12px] font-bold ${isAutoMode ? (isDark ? 'text-white' : 'text-black') : (isDark ? 'text-zinc-200' : 'text-black')}`}>
                    {currentMode.label}
                  </span>
                  {/* 🔥 AUTO MODE INDICATOR */}
                  {isAutoMode && (
