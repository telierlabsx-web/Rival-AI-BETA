...
// FIX: SEMUA ICON PAKAI SVG STROKE (NO EMOJI BERWARNA)
const Icons = {
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  return (
    /* FIX UTAMA: Container ini harus punya background solid (isDark) 
       biar riwayat chat yang scroll ke bawah "menghilang" di balik bar ini, 
       bukan kelihatan tumpang tindih (seperti di foto 76332.jpg).
    */
    <div className={`sticky bottom-0 w-full z-[100] ${isDark ? 'bg-[#0a0a0a]' : 'bg-white'} border-t ${isDark ? 'border-white/5' : 'border-black/5'} pb-[env(safe-area-inset-bottom)]`}>
      <div className="max-w-3xl mx-auto px-4 py-3">
        
        {/* POPUP ATTACHMENT */}
        {isAttachOpen && (
          <div className="absolute bottom-[85px] left-4 w-44 animate-in fade-in slide-in-from-bottom-2 duration-150">
            <div className={`${isDark ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-black/10'} rounded-2xl border shadow-2xl p-1.5`}>
              {[{ name: 'Camera', icon: <Icons.Camera /> }, { name: 'Photos', icon: <Icons.Photos /> }, { name: 'Files', icon: <Icons.Files /> }].map((item) => (
                <button key={item.name} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-zinc-800/50 text-[14px] ${isDark ? 'text-white' : 'text-black'}`}>
        ...
        {/* POPUP MODE */}
        {isModeOpen && (
          <div className="absolute bottom-[85px] left-4 w-[310px] animate-in fade-in slide-in-from-bottom-2 duration-150">
          ...
          <div className="flex flex-col p-2">
            ...
            <div className="flex items-center justify-between px-2 pb-1 mt-1">
              <div className="flex items-center gap-1.5">
                <button onClick={() => {setIsAttachOpen(!isAttachOpen); setIsModeOpen(false);}} className="p-2 text-zinc-500 hover:text-zinc-300 transition-colors">
                ...
                <button 
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full border ${isDark ? 'border-white/10 bg-white/5' : 'border-black/5 bg-black/5'}`}
                >
                  <span className="text-zinc-400">{currentMode.icon}</span>
                  <span className={`text-[12px] font-bold ${isDark ? 'text-zinc-200' : 'text-black'}`}>{currentMode.label}</span>
                  <svg className={`w-3.5 h-3.5 text-zinc-500 transition-transform ${isModeOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 9l-7 7-7-7" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round"/></svg>
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  ...
                  className={`w-9 h-9 flex items-center justify-center rounded-full transition-all ${input.trim() ? (isDark ? 'bg-white text-black' : 'bg-black text-white') : 'bg-zinc-800 text-zinc-600 opacity-50'}`}
                >
