  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        console.log(`🖼️ Compressed: ${width}x${height}, ~${sizeMB.toFixed(2)}MB`);
...
export const ChatInput: React.FC<ChatInputProps> = ({
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>, isCamera: boolean = false) => {
    ...
    if (availableSlots <= 0) {
      alert(`Maksimal ${maxImages} gambar ya bro!`);
      e.target.value = '';
      return;
    }
    ...
    const filesToProcess = Array.from(files)
      .filter(file => {
        if (file.size > 10 * 1024 * 1024) {
          alert(`${file.name} terlalu besar (max 10MB per file)`);
          return false;
        }
        return true;
      })
    ...
    try {
      ...
      console.log(`🔄 Compressing ${rawImages.length} images...`);
      const compressed = await Promise.all(
        rawImages.map(img => compressImage(img, 1024, 0.7))
      );
      
      console.log(`✅ Compression complete!`);
      
      if (setSelectedImages) {
        setSelectedImages([...(selectedImages || []), ...compressed]);
      }
    } catch (error) {
      console.error('❌ Error processing images:', error);
      alert('Gagal memproses gambar. Coba lagi atau pilih gambar yang lebih kecil.');
    } finally {
      ...
      <div className="max-w-3xl mx-auto px-4 pb-6 pt-4 pointer-events-auto relative">
          ...
          <div className="absolute bottom-full left-4 mb-2 w-44 animate-in fade-in slide-in-from-bottom-2 duration-150 z-[110]">
            <div className={`${isDark ? 'bg-[#1a1a1a] border-white/10' : 'bg-white border-black/10'} rounded-2xl border shadow-2xl p-1.5`}>
              <button 
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors ${isDark ? 'text-white hover:bg-zinc-800/50' : 'text-black hover:bg-zinc-100'} text-[14px] ${isCompressing ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
        ...
        <div className={`${isDark ? 'bg-[#121212] border-white/10' : 'bg-zinc-100 border-black/5'} rounded-[28px] border shadow-sm transition-all`}>
          <div className="flex flex-col p-2">
            ...
            {isCompressing && (
              <div className="px-4 py-2 flex items-center gap-2 text-zinc-500 text-sm">
            ...
            <textarea
              ...
              disabled={isCompressing}
              className="w-full bg-transparent px-4 py-2 focus:outline-none text-[16px] text-inherit placeholder-zinc-500 resize-none overflow-y-auto disabled:opacity-50"
              rows={1}
            />
            ...
            <div className="flex items-center justify-between px-2 pb-1 mt-1">
              ...
              <div className="flex items-center gap-2">
                ...
                <button 
                  onClick={onSend}
                  disabled={!input.trim() || isLoading || isCompressing}
                  className={`w-9 h-9 flex items-center justify-center rounded-full transition-all ${
                    input.trim() && !isLoading && !isCompressing
