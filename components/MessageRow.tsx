...
export const MessageRow: React.FC<MessageRowProps> = ({
  ...
  // 🔥 Deteksi Canvas mode dari message metadata (dari geminiService response)
  const shouldShowArtifact = (message as any).shouldShowArtifactCard || false;
  ...
  return (
    <div className={`w-full flex justify-center py-10 md:py-16 border-b ${borderColor} ${message.role === 'assistant' ? assistantRowBg : 'bg-transparent'}`}>
      <div className="w-full max-w-4xl px-6">
        ...
        {/* ✅ LOGO + NAMA + WAKTU (SEMUA HORIZONTAL SATU BARIS) */}
        <div className="flex items-center gap-3 mb-4">
          <img 
            src={message.role === 'assistant' ? profile.aiAvatar : profile.avatar} 
            className={`w-10 h-10 md:w-12 md:h-12 rounded-xl object-cover shadow-sm ${borderColor} border flex-shrink-0`} 
            alt={message.role} 
          />
          <span className={`text-base md:text-lg font-bold ${isDark ? 'text-zinc-200' : 'text-zinc-900'}`}>
            {message.role === 'assistant' ? profile.aiName : 'Anda'}
          </span>
          <span className={`text-xs ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
            {format(message.timestamp, 'EEEE, dd MMM yyyy · HH:mm')}
          </span>
        </div>

        {/* ✅ GARIS PEMISAH - CUMA BUAT AI */}
        {message.role === 'assistant' && (
          <div className={`w-full h-px mb-6 ${isDark ? 'bg-zinc-800' : 'bg-zinc-200'}`} />
        )}
