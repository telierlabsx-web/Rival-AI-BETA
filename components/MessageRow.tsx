...
export const MessageRow: React.FC<MessageRowProps> = ({
  ...
  return (
    // 🔥 FIX: Reduced padding, better spacing
    <div className={`w-full flex justify-center py-6 md:py-8 border-b ${borderColor} ${message.role === 'assistant' ? assistantRowBg : 'bg-transparent'}`}>
      <div className="w-full max-w-4xl mx-auto px-4 md:px-6">
        
        {/* 🔥 FIX: Flexbox layout dengan gap konsisten */}
        <div className="flex items-start gap-3 md:gap-4 mb-3">
          <img 
            src={message.role === 'assistant' ? profile.aiAvatar : profile.avatar} 
            className={`w-9 h-9 md:w-10 md:h-10 rounded-xl object-cover shadow-sm ${borderColor} border flex-shrink-0`} 
            alt={message.role} 
          />
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 flex-wrap mb-2">
              <span className={`text-sm md:text-base font-bold ${isDark ? 'text-zinc-200' : 'text-zinc-900'}`}>
                {message.role === 'assistant' ? profile.aiName : 'Anda'}
              </span>
              <span className={`text-[10px] md:text-xs ${isDark ? 'text-zinc-500' : 'text-zinc-400'}`}>
            ...
            {/* 🔥 FIX: Content wrapper dengan max-w + overflow handling */}
            <div className="w-full max-w-full overflow-hidden">
                ...
                <div className="markdown-content break-words overflow-wrap-anywhere">
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      // 🔥 FIX: Code blocks dengan PROPER overflow + wrap
              ...
              {/* 🔥 FIX: Artifact Card dengan max-w */}
