...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="px-3 pt-6 pb-2 space-y-1">
        <button 
          onClick={onNewChat}
        ...
        <button 
          ...
          className={`w-full flex items-center gap-3 px-3.5 py-3 rounded-xl transition-all ${isDark ? 'hover:bg-zinc-800 text-zinc-300' : 'hover:bg-zinc-100 text-zinc-600'}`}
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-3 space-y-6 pb-20 pt-4">
      ...
      <div className={`mt-auto p-4 border-t ${borderColor}`}>
        <button 
          ...
          className={`w-full flex items-center gap-3 p-2.5 rounded-2xl transition-all group ${isDark ? 'hover:bg-zinc-800' : 'hover:bg-zinc-50'}`}
        >
          <img src={userProfile.avatar} alt="P" className={`w-9 h-9 rounded-full border ${borderColor} object-cover`} />
          <div className="text-left flex-1 min-w-0">
            <p className={`text-xs font-black truncate ${textColor}`}>{userProfile.name}</p>
            <p className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Pengaturan Rival</p>
          </div>
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="space-y-1">
        {items.map(item => {
          const timeText = format(item.updatedAt, 'EEEE, dd MMM yyyy · HH:mm').toUpperCase();
          ...
          return (
            <div key={item.id} className="group relative">
              ...
              >
                ...
                <span className={`text-[8px] font-black tracking-wider ${activeId === item.id ? (isDark ? 'opacity-40' : 'opacity-40') : 'opacity-30'}`}>
                  {timeText}
                </span>
              </button>
              <button 
