...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ 
  ...
  const handleScroll = () => {
    ...
    // Reset flag after 1s of no scrolling
  ...
  // 🔥 AUTO-SCROLL saat AI typing (HANYA kalau user ga lagi scroll manual)
  useEffect(() => {
  ...
  // 🔥 SCROLL saat kirim message baru
  useEffect(() => {
    if (session?.messages && session.messages.length > 0) {
      const lastMsg = session.messages[session.messages.length - 1];
      if (lastMsg.role === 'user') {
        scrollToBottom('auto'); // Instant scroll for user messages
      }
    }
  ...
  return (
    <div className="flex-1 flex flex-col h-screen md:h-[100dvh] relative overflow-hidden">
      ...
      <div 
        ...
        className="flex-1 overflow-y-auto no-scrollbar pt-2 pb-56 relative"
      >
        ...
        {showScrollButton && (
          <button
            ...
            className={`fixed bottom-72 right-6 z-50 w-10 h-10 md:w-11 md:h-11 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 ${
              isDark 
                ? 'bg-zinc-800 hover:bg-zinc-700 text-white border border-white/10' 
                : 'bg-white hover:bg-zinc-50 text-black border border-black/10'
